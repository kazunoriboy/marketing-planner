...
【コントラスト品質原則（必須）】
以下のルールはいかなるデザイン選択よりも優先される Non-negotiable 要件です。

1. 画像上のテキスト
   - 画像の上にテキストを重ねる場合、必ず rgba オーバーレイを使うこと
   - 推奨: rgba(0, 0, 0, 0.45) 以上（暗い画像なら rgba(0, 0, 0, 0.3) でも可）
   - 明るいテキストを使う場合は暗いオーバーレイ、暗いテキストを使う場合は白系オーバーレイ

2. テキスト色のコントラスト比
   - 本文テキスト（18px 以下）: 背景との比率 4.5:1 以上（WCAG AA 基準）
   - 大見出し（24px 以上 / Bold）: 3:1 以上（WCAG AA 大テキスト基準）
   - 薄いグレー（#999 以下）を小さい本文テキストに使用しないこと

3. CTA ボタン
   - ボタン背景色とボタンテキスト色のコントラスト比を 4.5:1 以上に保つこと"""
...
.hero::after {{
  ...
  position: absolute;
  inset: 0;
  background: rgba(0, 0, 0, 0.45);  /* 明るい写真は 0.5 以上に調整 */
}}
...
■ 本文テキストの最低コントラスト
- 白背景: #595959 以上の暗さ（比率 7:1）を推奨、最低でも #767676（4.5:1）
- ダーク背景: #a0a0a0 以上の明るさを確保すること

■ フッターセクション
- ダーク系背景（#1a1a1a〜#333333 等）を推奨し、テキストは #e0e0e0 以上の明るさを使うこと
- リンクテキストは通常テキストより明るめ（#ffffff 推奨）にしてアクセシビリティを確保
- 著作権表示などの小さいテキスト（12〜14px）は #b0b0b0 以上を使い、コントラスト比 4.5:1 以上を維持すること
- フッター内の CTA ボタンは背景色との差を明確にし、ボーダーを付ける場合は 2px 以上の太さで視認性を確保
