...
1. 画像上のテキスト視認性（オーバーレイ必須）
   - ヒーローエリアや画像カードなど、背景に画像を使用する場合は、文字の背面に必ずオーバーレイを適用すること。
   - 実装: .hero::after 等で rgba(0,0,0,0.45) 以上の暗幕を重ねるか、テキスト周辺に linear-gradient（下から上への黒グラデーション）を配置し、「逆光で文字が消える」現象を物理的に防ぐこと。

2. 背景色に応じたダイナミック・コントラスト
   - 背景色を解析し、文字色（Color）を以下のルールで自動選択すること。
     (a) 暗い背景（Dark mode系）: 明るめの文字（白、または視認性の高いライトグレー）を選択。
     (b) 明るい背景（Light mode系）: 暗めの文字（黒、または濃いグレー）を選択。
   - 上記(a)(b)の原則が守られている場合に限り、デザイン上の意図として「薄いグレー」等のニュアンスカラーの使用を許可する。

3. CTAボタンとフッターの構造
   - CTAボタン: 背景色に対して最も誘目性が高く、かつコントラスト比 4.5:1 以上を維持する色を動的に選択すること（特定のカラーコード固定は不要）。
   - フッター: セクションの区切りを明確にするため、原則として「ダーク背景」を推奨する。その際、テキストは背景に沈まない明度を維持すること。"""
