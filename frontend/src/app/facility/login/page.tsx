...
export default function FacilityLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await facilityApi.login({ email, password });
      ...
      <div className="max-w-md w-full">
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <div className="text-center mb-8">
            <div className="mx-auto h-16 w-16 flex items-center justify-center rounded-full bg-teal-100">
              <Hotel className="h-10 w-10 text-teal-600" />
            </div>
            <h2 className="mt-4 text-2xl font-bold text-gray-900">
              施設管理者ログイン
            </h2>
            <p className="mt-2 text-sm text-gray-600">
              施設管理者アカウントでログインしてください
            </p>
          </div>
